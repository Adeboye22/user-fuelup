...
import { Bell, Check, AlertTriangle, Trash2, MoreHorizontal, ShoppingCart, FileText } from "lucide-react"
          ...
          <>
            ...
            {(pagination.hasNextPage || pagination.hasPreviousPage) && (
              <div className="flex justify-center gap-4 mt-6">
                <Button variant="outline" size="sm" onClick={loadPrevPage} disabled={!pagination.hasPreviousPage}>
                  Previous
                </Button>
                <span className="flex items-center text-sm text-gray-500">
                  Page {pagination.currentPage} of {pagination.totalPages}
                </span>
                <Button variant="outline" size="sm" onClick={loadNextPage} disabled={!pagination.hasNextPage}>
                  Next
                </Button>
              </div>
            )}
...
const NotificationItem = ({ notification, getNotificationIcon, getNotificationColor, onMarkAsRead, onDelete }) => {
  return (
    <motion.div
      ...
      animate={{ opacity: 1, y: 0 }}
      className={`border ${
        notification.read ? "border-gray-100 dark:border-gray-700/50" : "border-emerald-200 dark:border-emerald-700/50"
      } rounded-lg p-4 ${
        notification.read ? "bg-white dark:bg-gray-800/40" : "bg-emerald-50/50 dark:bg-emerald-900/10"
      } transition-colors`}
    >
          ...
          <div className="flex items-start justify-between">
            <h3
              className={`font-medium ${
                notification.read ? "text-gray-800 dark:text-white" : "text-emerald-700 dark:text-emerald-400"
              }`}
            >
